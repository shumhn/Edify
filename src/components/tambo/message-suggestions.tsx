...
export interface MessageSuggestionsProps extends React.HTMLAttributes<HTMLDivElement> {
  ...
  /** Whether to show static, dynamic, or combined suggestions */
  mode?: "static" | "dynamic" | "combined";
}
...
>(
  (
    ...
    const suggestionLimit = mode === "static" ? 0 : maxSuggestions;
    const {
      suggestions: generatedSuggestions,
      selectedSuggestionId,
      accept,
      generateResult: { isPending: isGenerating, error },
    } = useTamboSuggestions({ maxSuggestions: suggestionLimit });
    ...
    const suggestions = React.useMemo(() => {
      ...
      if (mode === "combined") {
        return [...initialSuggestions, ...generatedSuggestions].slice(
          0,
          maxSuggestions,
        );
      }
      if (generatedSuggestions.length > 0) {
        return generatedSuggestions.slice(0, maxSuggestions);
      }
    ...
    const contextValue = React.useMemo(
      () => ({
        ...
        selectedSuggestionId,
        accept,
        isGenerating: mode === "static" ? false : isGenerating,
        error: mode === "static" ? null : error,
        thread,
        isMac,
        mode,
      }),
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "flex flex-wrap gap-2 overflow-x-auto pb-2 rounded-md bg-transparent min-h-[2.5rem]",
        ...
        : // Render placeholder buttons when no suggestions are available
          placeholders.map((_, index) => (
            <div
              ...
              className="px-3 py-1.5 rounded-full text-xs border border-slate-200 bg-slate-100 text-transparent animate-pulse"
...
function getSuggestionButtonClassName({
  ...
  return "bg-white text-slate-700 border-slate-200 hover:bg-slate-50 hover:text-slate-900 hover:border-slate-300";
}
